...
db=dbConnect(dbDriver("SQLite"), "/Users/xiaoweichen/Kaggle/ClintonEmails/data/database.sqlite")

# Extract email text from emails sent by Hilary
emails=dbGetQuery(db, "SELECT ExtractedBodyText EmailBody FROM Emails e INNER JOIN Persons p 
                     ON e.SenderPersonId=P.Id WHERE p.Name='Hillary Clinton'  AND 
...
# Remove the words that have 2 or 3 characters
...
for (n in 1:nrow(emails)) {
  ...
  # How many times is a specific country mentioned in the email text?
...
# Summarize the sentiment by country
